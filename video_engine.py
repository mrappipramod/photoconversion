"""
video_engine.py
All ffmpeg/ffprobe logic lives here. Each function is self-contained,
returns (output_path, error_string). output_path is None on failure.
"""
import os, subprocess, tempfile, json, shutil

FONT = "/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf"

FILTERS = {
    "None":        None,
    "Vivid":       "eq=saturation=1.8:contrast=1.1:brightness=0.05",
    "Cinematic":   "eq=saturation=0.85:contrast=1.2:brightness=-0.03",
    "Warm Sunset": "eq=saturation=1.3:brightness=0.04",
    "Cool Blue":   "eq=saturation=1.1:brightness=-0.02",
    "Black & White":"colorchannelmixer=.299:.587:.114:0:.299:.587:.114:0:.299:.587:.114",
    "Vintage":     "eq=saturation=0.7,curves=r='0/0.1 0.5/0.55 1/0.9'",
    "HDR Pop":     "eq=saturation=2.0:contrast=1.3:brightness=0.02",
    "Matte":       "eq=saturation=0.85,curves=r='0/0.08 1/0.9'",
    "Neon Night":  "eq=saturation=2.2:contrast=1.4:brightness=-0.05",
}

PLATFORMS = {
    "Instagram Reels (9:16)": {"w": 1080, "h": 1920, "fps": 30},
    "YouTube Shorts (9:16)":  {"w": 1080, "h": 1920, "fps": 60},
    "YouTube (16:9)":         {"w": 1920, "h": 1080, "fps": 30},
    "TikTok (9:16)":          {"w": 1080, "h": 1920, "fps": 30},
    "Square (1:1)":           {"w": 1080, "h": 1080, "fps": 30},
}

# ─── helpers ──────────────────────────────────────────────────────

def _tmp(ext=".mp4"):
    t = tempfile.NamedTemporaryFile(delete=False, suffix=ext)
    t.close()
    return t.name

def _run(*args):
    cmd = ["ffmpeg", "-y"] + [str(a) for a in args]
    r = subprocess.run(cmd, capture_output=True, text=True)
    return r.returncode == 0, r.stderr

def _ok(path):
    return path and os.path.exists(path) and os.path.getsize(path) > 1000

def probe(path):
    """Return basic video metadata dict."""
    r = subprocess.run(
        ["ffprobe", "-v", "quiet", "-print_format", "json",
         "-show_streams", "-show_format", path],
        capture_output=True, text=True
    )
    info = {"duration": 0.0, "width": 0, "height": 0, "fps": 30.0, "has_audio": False, "size_mb": 0.0}
    if r.returncode != 0:
        return info
    try:
        d = json.loads(r.stdout)
        for s in d.get("streams", []):
            if s.get("codec_type") == "video":
                info["width"]  = int(s.get("width", 0))
                info["height"] = int(s.get("height", 0))
                try:
                    n, dv = s.get("r_frame_rate", "30/1").split("/")
                    info["fps"] = round(int(n) / max(int(dv), 1), 2)
                except Exception:
                    pass
                raw_dur = s.get("duration") or d.get("format", {}).get("duration", 0)
                info["duration"] = float(raw_dur or 0)
            if s.get("codec_type") == "audio":
                info["has_audio"] = True
        info["size_mb"] = round(int(d.get("format", {}).get("size", 0)) / 1_048_576, 1)
    except Exception:
        pass
    return info

# ─── individual step renderers ────────────────────────────────────

def do_trim(inp, start, end):
    out = _tmp()
    ok, e = _run(
        "-ss", start, "-i", inp,
        "-t", end - start,
        "-c:v", "libx264", "-c:a", "aac", "-preset", "ultrafast", out
    )
    return (out, None) if _ok(out) else (None, e)


def do_filter(inp, filter_name, brightness=0.0, contrast=1.0, saturation=1.0):
    out = _tmp()
    parts = []
    base = FILTERS.get(filter_name)
    if base:
        parts.append(base)
    # Only add eq adjustment if values differ from neutral
    if abs(brightness) > 0.001 or abs(contrast - 1.0) > 0.01 or abs(saturation - 1.0) > 0.01:
        parts.append(f"eq=brightness={brightness:.3f}:contrast={contrast:.3f}:saturation={saturation:.3f}")
    vf = ",".join(parts) if parts else "null"
    ok, e = _run("-i", inp, "-vf", vf, "-c:v", "libx264", "-c:a", "aac", "-preset", "ultrafast", out)
    return (out, None) if _ok(out) else (None, e)


def do_speed(inp, speed):
    out = _tmp()
    info = probe(inp)
    pts = f"setpts={1.0/speed:.6f}*PTS"
    if info["has_audio"]:
        # chain atempo filters — each must be 0.5–2.0
        parts = []
        rem = float(speed)
        while rem > 2.0:
            parts.append("atempo=2.0")
            rem /= 2.0
        while rem < 0.5:
            parts.append("atempo=0.5")
            rem *= 2.0
        parts.append(f"atempo={rem:.4f}")
        af = ",".join(parts)
        ok, e = _run("-i", inp, "-vf", pts, "-af", af,
                     "-c:v", "libx264", "-preset", "ultrafast", out)
    else:
        ok, e = _run("-i", inp, "-vf", pts, "-an",
                     "-c:v", "libx264", "-preset", "ultrafast", out)
    return (out, None) if _ok(out) else (None, e)


def do_text(inp, text, style="Custom", animation="None",
            font_size=48, color="#FFFFFF",
            x_pct=5, y_pct=80, start=0.5, end=4.0,
            shadow=True, bg_box=False, bg_color="#000000"):
    """Burn text overlay. Uses safe drawtext expressions."""
    out = _tmp()
    info = probe(inp)
    dur  = info["duration"] or 10.0

    end = min(float(end), dur)
    start = float(start)

    # Sanitise text — escape special chars for ffmpeg drawtext
    safe_text = (text
                 .replace("\\", "\\\\")
                 .replace("'",  "\u2019")   # curly apostrophe — avoids shell quoting issues
                 .replace(":",  "\\:")
                 .replace("%",  "\\%"))

    # Color — ensure 6-char hex
    hex_col = color.lstrip("#")
    if len(hex_col) != 6:
        hex_col = "FFFFFF"
    fc = f"0x{hex_col.upper()}FF"

    # Position — plain pixel math, no w/h expressions in alpha/y
    # We use fixed-string enable instead of alpha animation for reliability
    enable_expr = f"between(t\\,{start:.3f}\\,{end:.3f})"

    # Font arg
    font_arg = f":fontfile={FONT}" if os.path.exists(FONT) else ""

    # Shadow / box
    shadow_arg = ":shadowcolor=black@0.7:shadowx=2:shadowy=2" if shadow else ""
    box_arg    = ""
    if bg_box:
        bh = bg_color.lstrip("#")
        if len(bh) != 6:
            bh = "000000"
        box_arg = f":box=1:boxcolor=0x{bh.upper()}99:boxborderw=10"

    # Position expressions — simple multiplication avoids parser issues
    x_expr = f"(w*{x_pct/100:.4f})"
    y_expr = f"(h*{y_pct/100:.4f})"

    # Build the drawtext filter
    dt = (f"drawtext=text='{safe_text}'"
          f":fontsize={font_size}"
          f"{font_arg}"
          f":fontcolor={fc}"
          f":x={x_expr}:y={y_expr}"
          f":enable='{enable_expr}'"
          f"{shadow_arg}{box_arg}")

    # Style overlays (coloured bars behind text)
    if style == "Lower Third":
        bar = (f"drawbox=x=0"
               f":y=(h*{y_pct/100:.4f})-{font_size+16}"
               f":w=iw:h={font_size+32}"
               f":color=0x00000099:t=fill"
               f":enable='{enable_expr}'")
        vf = f"{bar},{dt}"
    elif style == "News Ticker":
        bar = (f"drawbox=x=0"
               f":y=(h*{y_pct/100:.4f})-8"
               f":w=iw:h={font_size+20}"
               f":color=0xe11d48CC:t=fill"
               f":enable='{enable_expr}'")
        vf = f"{bar},{dt}"
    elif style == "Title Card":
        bar = (f"drawbox=x=(w*{x_pct/100:.4f})-20"
               f":y=(h*{y_pct/100:.4f})-{font_size+8}"
               f":w=iw/2:h={font_size+20}"
               f":color=0x000000BB:t=fill"
               f":enable='{enable_expr}'")
        vf = f"{bar},{dt}"
    else:
        vf = dt

    ok, e = _run("-i", inp, "-vf", vf,
                 "-c:v", "libx264", "-c:a", "aac", "-preset", "ultrafast", out)
    return (out, None) if _ok(out) else (None, e)


def do_watermark_text(inp, text="@yourbrand", position="Bottom Right", opacity=0.75, size=28):
    out = _tmp()
    safe = (text.replace("\\", "\\\\").replace("'", "\u2019").replace(":", "\\:"))
    pos_map = {
        "Top Left":     ("20",            "20"),
        "Top Right":    ("w-tw-20",       "20"),
        "Bottom Left":  ("20",            "h-th-20"),
        "Bottom Right": ("w-tw-20",       "h-th-20"),
        "Center":       ("(w-tw)/2",      "(h-th)/2"),
    }
    # Use text_w / text_h aliases
    x_map = {
        "Top Left":     "20",
        "Top Right":    "w-text_w-20",
        "Bottom Left":  "20",
        "Bottom Right": "w-text_w-20",
        "Center":       "(w-text_w)/2",
    }
    y_map = {
        "Top Left":     "20",
        "Top Right":    "20",
        "Bottom Left":  "h-text_h-20",
        "Bottom Right": "h-text_h-20",
        "Center":       "(h-text_h)/2",
    }
    x = x_map.get(position, "w-text_w-20")
    y = y_map.get(position, "h-text_h-20")
    font_arg = f":fontfile={FONT}" if os.path.exists(FONT) else ""
    vf = (f"drawtext=text='{safe}':fontsize={size}{font_arg}"
          f":fontcolor=white@{opacity:.2f}:x={x}:y={y}"
          f":shadowcolor=black@0.6:shadowx=2:shadowy=2")
    ok, e = _run("-i", inp, "-vf", vf,
                 "-c:v", "libx264", "-c:a", "aac", "-preset", "ultrafast", out)
    return (out, None) if _ok(out) else (None, e)


def do_watermark_logo(inp, logo_path, position="Bottom Right", opacity=0.75, size_pct=15):
    out = _tmp()
    pos_map = {
        "Top Left":     "10:10",
        "Top Right":    "main_w-overlay_w-10:10",
        "Bottom Left":  "10:main_h-overlay_h-10",
        "Bottom Right": "main_w-overlay_w-10:main_h-overlay_h-10",
        "Center":       "(main_w-overlay_w)/2:(main_h-overlay_h)/2",
    }
    ovp = pos_map.get(position, "main_w-overlay_w-10:main_h-overlay_h-10")
    scale = size_pct / 100.0
    vf = (f"[1:v]scale=iw*{scale:.3f}:-1,format=rgba,"
          f"colorchannelmixer=aa={opacity:.2f}[logo];"
          f"[0:v][logo]overlay={ovp}")
    ok, e = _run("-i", inp, "-i", logo_path,
                 "-filter_complex", vf,
                 "-c:v", "libx264", "-c:a", "aac", "-preset", "ultrafast", out)
    return (out, None) if _ok(out) else (None, e)


def do_progress_bar(inp, color="#e11d48", height=8, position="bottom"):
    out  = _tmp()
    info = probe(inp)
    dur  = info["duration"] or 1.0
    hex_col = color.lstrip("#")
    if len(hex_col) != 6:
        hex_col = "e11d48"
    y = f"h-{height}" if position == "bottom" else "0"
    vf = (f"drawbox=x=0:y={y}"
          f":w=iw*(t/{dur:.4f}):h={height}"
          f":color=0x{hex_col.upper()}FF:t=fill")
    ok, e = _run("-i", inp, "-vf", vf,
                 "-c:v", "libx264", "-c:a", "aac", "-preset", "ultrafast", out)
    return (out, None) if _ok(out) else (None, e)


def do_kenburns(inp, zoom_start=1.0, zoom_end=1.08):
    out  = _tmp()
    info = probe(inp)
    w    = info["width"]  or 1080
    h    = info["height"] or 1920
    fps  = info["fps"]    or 30.0
    dur  = info["duration"] or 5.0
    frames = max(int(dur * fps), 1)
    step   = (zoom_end - zoom_start) / frames
    vf = (f"scale=iw*{zoom_end:.3f}:ih*{zoom_end:.3f},"
          f"zoompan=z='min(zoom+{step:.6f},{zoom_end:.4f})':"
          f"d={frames}:"
          f"x='iw/2-(iw/zoom/2)':"
          f"y='ih/2-(ih/zoom/2)':"
          f"s={w}x{h}:fps={fps:.2f}")
    ok, e = _run("-i", inp, "-vf", vf,
                 "-c:v", "libx264", "-c:a", "aac", "-preset", "ultrafast", out)
    return (out, None) if _ok(out) else (None, e)


def do_audio_mute(inp):
    out = _tmp()
    ok, e = _run("-i", inp, "-c:v", "copy", "-an", out)
    return (out, None) if _ok(out) else (None, e)


def do_audio_volume(inp, multiplier=1.5):
    out = _tmp()
    ok, e = _run("-i", inp, "-af", f"volume={multiplier:.2f}", "-c:v", "copy", out)
    return (out, None) if _ok(out) else (None, e)


def do_audio_replace(inp, audio_path):
    out = _tmp()
    ok, e = _run("-i", inp, "-i", audio_path,
                 "-c:v", "copy",
                 "-map", "0:v:0", "-map", "1:a:0",
                 "-shortest", out)
    return (out, None) if _ok(out) else (None, e)


def do_audio_mix(inp, audio_path, music_vol=0.5, orig_vol=1.0):
    out = _tmp()
    fc  = (f"[0:a]volume={orig_vol:.2f}[a1];"
           f"[1:a]volume={music_vol:.2f}[a2];"
           f"[a1][a2]amix=inputs=2:duration=first[ao]")
    ok, e = _run("-i", inp, "-i", audio_path,
                 "-filter_complex", fc,
                 "-map", "0:v", "-map", "[ao]",
                 "-c:v", "copy", "-shortest", out)
    return (out, None) if _ok(out) else (None, e)


def do_export(inp, platform="YouTube (16:9)", quality="High", fit="crop"):
    out  = _tmp()
    spec = PLATFORMS.get(platform, {"w": 1920, "h": 1080, "fps": 30})
    w, h = spec["w"], spec["h"]
    crf  = {"Draft": 32, "Standard": 26, "High": 20, "Max": 16}.get(quality, 20)
    if fit == "crop":
        vf = (f"scale={w}:{h}:force_original_aspect_ratio=increase,"
              f"crop={w}:{h}")
    else:
        vf = (f"scale={w}:{h}:force_original_aspect_ratio=decrease,"
              f"pad={w}:{h}:(ow-iw)/2:(oh-ih)/2:black")
    ok, e = _run("-i", inp,
                 "-vf", vf,
                 "-r", str(spec["fps"]),
                 "-c:v", "libx264", "-crf", str(crf), "-preset", "fast",
                 "-c:a", "aac", "-b:a", "192k", out)
    return (out, None) if _ok(out) else (None, e)


def do_merge(paths):
    """Concatenate a list of video paths into one."""
    if len(paths) == 1:
        out = _tmp()
        shutil.copy2(paths[0], out)
        return out, None
    # Normalise all clips first
    normed = []
    for p in paths:
        n = _tmp()
        ok, e = _run("-i", p, "-c:v", "libx264", "-c:a", "aac", "-preset", "ultrafast", n)
        if not _ok(n):
            return None, f"Normalise failed: {e[-200:]}"
        normed.append(n)
    lst = tempfile.NamedTemporaryFile(delete=False, suffix=".txt", mode="w")
    for n in normed:
        lst.write(f"file '{n}'\n")
    lst.close()
    out = _tmp()
    ok, e = _run("-f", "concat", "-safe", "0", "-i", lst.name,
                 "-c:v", "libx264", "-c:a", "aac", "-preset", "ultrafast", out)
    os.unlink(lst.name)
    return (out, None) if _ok(out) else (None, e)


def extract_frame(path, ts=1.0):
    """Return PIL Image of a frame, or None."""
    from PIL import Image
    info = probe(path)
    ts   = min(float(ts), max(0.0, info["duration"] - 0.1))
    out  = _tmp(".jpg")
    ok, _ = _run("-ss", str(ts), "-i", path, "-vframes", "1", "-q:v", "2", out)
    if ok and _ok(out):
        try:
            img = Image.open(out)
            img.load()
            return img
        except Exception:
            pass
    return None


# ─── pipeline runner ──────────────────────────────────────────────

def run_pipeline(source_path, steps, audio_path=None, logo_path=None):
    """
    Execute a list of step dicts sequentially.
    Returns (final_path, log_list).  final_path is None on failure.
    """
    import shutil
    cur = _tmp()
    shutil.copy2(source_path, cur)
    log = []

    for i, step in enumerate(steps):
        t   = step.get("type", "")
        num = f"[{i+1}/{len(steps)}]"

        if t == "trim":
            result, err = do_trim(cur, step["start"], step["end"])

        elif t == "filter":
            result, err = do_filter(
                cur,
                step.get("filter", "None"),
                step.get("brightness", 0.0),
                step.get("contrast",   1.0),
                step.get("saturation", 1.0),
            )

        elif t == "speed":
            result, err = do_speed(cur, step["speed"])

        elif t == "text":
            result, err = do_text(
                cur,
                text       = step.get("text", "Text"),
                style      = step.get("style", "Custom"),
                animation  = step.get("animation", "None"),
                font_size  = step.get("font_size", 48),
                color      = step.get("color", "#FFFFFF"),
                x_pct      = step.get("x_pct", 5),
                y_pct      = step.get("y_pct", 80),
                start      = step.get("start", 0.5),
                end        = step.get("end", 4.0),
                shadow     = step.get("shadow", True),
                bg_box     = step.get("bg_box", False),
                bg_color   = step.get("bg_color", "#000000"),
            )

        elif t == "watermark_text":
            result, err = do_watermark_text(
                cur,
                text     = step.get("text", "@brand"),
                position = step.get("position", "Bottom Right"),
                opacity  = step.get("opacity", 0.75),
                size     = step.get("size", 28),
            )

        elif t == "watermark_logo":
            if logo_path:
                result, err = do_watermark_logo(
                    cur, logo_path,
                    position  = step.get("position", "Bottom Right"),
                    opacity   = step.get("opacity", 0.75),
                    size_pct  = step.get("size_pct", 15),
                )
            else:
                result, err = cur, None  # skip if no logo uploaded

        elif t == "progress_bar":
            result, err = do_progress_bar(
                cur,
                color    = step.get("color", "#e11d48"),
                height   = step.get("height", 8),
                position = step.get("position", "bottom"),
            )

        elif t == "kenburns":
            result, err = do_kenburns(
                cur,
                zoom_start = step.get("zoom_start", 1.0),
                zoom_end   = step.get("zoom_end",   1.08),
            )

        elif t == "audio_mute":
            result, err = do_audio_mute(cur)

        elif t == "audio_volume":
            result, err = do_audio_volume(cur, step.get("multiplier", 1.5))

        elif t == "audio_replace":
            if audio_path:
                result, err = do_audio_replace(cur, audio_path)
            else:
                result, err = cur, None

        elif t == "audio_mix":
            if audio_path:
                result, err = do_audio_mix(
                    cur, audio_path,
                    music_vol = step.get("music_vol", 0.5),
                    orig_vol  = step.get("orig_vol",  1.0),
                )
            else:
                result, err = cur, None

        elif t == "export":
            result, err = do_export(
                cur,
                platform = step.get("platform", "YouTube (16:9)"),
                quality  = step.get("quality", "High"),
                fit      = step.get("fit", "crop"),
            )

        else:
            log.append(f"{num} ⚠️  Unknown step '{t}' — skipped")
            continue

        if result is None:
            log.append(f"{num} ❌ {t} failed: {(err or '')[-300:]}")
            return None, log

        log.append(f"{num} ✅ {t}")
        cur = result

    return cur, log
